...
export default function CategoryNav() {
  ...
  const [selectedParentCategory, setSelectedParentCategory] = useState<number>(MOCK_CATEGORY_TREE[0]?.id);

  const currentSubCategories = MOCK_CATEGORY_TREE.find(
  ...
  return (
    ...
    >
        ...
        <div className="flex items-center gap-x-8 h-12">
          ...
          >
            ...
            >
              ...
              <ChevronDown size={14} className={`transition-transform duration-200 ${activeMenu === 'categories' ? 'rotate-180' : ''}`} />
            </button>
          </div>
          ...
          >
            ...
            >
              ...
              <ChevronDown size={14} className={`transition-transform duration-200 ${activeMenu === 'brands' ? 'rotate-180' : ''}`} />
            </button>
          </div>
      ...
      {/* ============================================================
          CẤP 2 - PANEL DROPDOWN HIỂN THỊ CHI TIẾT KHI HOVER
          ============================================================ */}
          ...
          <div className="mx-auto max-w-7xl grid grid-cols-4 min-h-[280px]">
            ...
            <div className="col-span-1 bg-slate-50 border-r border-gray-100 py-4">
              {MOCK_CATEGORY_TREE.map((category) => (
                ...
                >
                  ...
                  <ArrowRight size={14} className={selectedParentCategory === category.id ? 'opacity-100' : 'opacity-0'} />
                </div>
              ))}
            </div>
            ...
            <div className="col-span-3 p-6 bg-white grid grid-cols-3 gap-6 content-start">
              {currentSubCategories.length > 0 ? (
                ...
                <p className="text-sm text-slate-400 col-span-3 italic">Ngành hàng này đang được cập nhật sản phẩm con.</p>
              )}
            </div>
      ...
      {/* 2.2 BẢNG CHỨA LƯỚI LOGO THƯƠNG HIỆU ĐỐI TÁC CHÍNH HÃNG (BẢNG BRANDS) */}
      {activeMenu === 'brands' && (
        <div className="absolute top-12 left-0 right-0 bg-white border-b border-gray-200 shadow-xl animate-in fade-in slide-in-from-top-2 duration-150 z-50">
          <div className="mx-auto max-w-7xl p-6">
            <div className="flex items-center justify-between mb-4 border-b border-gray-100 pb-2">
              <span className="text-xs font-black text-slate-400 uppercase tracking-wider">Thương hiệu phân phối độc quyền</span>
              <Link href="/brands" onClick={() => setActiveMenu(null)} className="text-xs font-bold text-indigo-600 hover:text-indigo-700">Xem tất cả hãng gia nhập →</Link>
            </div>
              ...
              {MOCK_BRANDS.map((brand) => (
                ...
                >
                  <div className="h-8 w-full flex items-center justify-center">
                    <img src={brand.logo_url} alt={brand.name} className="max-w-full max-h-full object-contain transition-transform group-hover:scale-102" />
                  </div>
