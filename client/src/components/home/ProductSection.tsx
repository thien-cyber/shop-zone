...
export default function ProductSection({ title, subtitle, products, viewAllLink, bgGray = false }: ProductSectionProps) {
  ...
  return (
    <section className={`py-12 sm:py-16 border-b border-gray-100 ${bgGray ? 'bg-slate-50/70' : 'bg-white'}`}>
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        
        <div className="flex items-end justify-between mb-8 border-l-4 border-indigo-600 pl-3">
          <div>
            <h3 className="text-lg sm:text-xl font-extrabold text-slate-900 tracking-tight">{title}</h3>
            {subtitle && <p className="text-xs text-slate-400 mt-0.5 hidden sm:block">{subtitle}</p>}
          </div>
          <Link href={viewAllLink} className="text-xs font-bold text-indigo-600 hover:text-indigo-700 whitespace-nowrap group transition">
        ...
        {/* [HÀNH ĐỘNG MỚI]: Nút bấm lớn căn giữa xuất hiện khi lượng hàng vượt quá 8 */}
        {hasMore && (
